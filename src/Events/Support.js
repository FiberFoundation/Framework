...
function stopListening(obj, name, callback) {
  ...
  for (var i = 0; i < ids.length; i++) {
    ...
    off.call(this, name, callback, this);
  }
...
function once(name, callback, context) {
  ...
  var events = eventsApi(onceMap, {}, name, callback, _.bind(off, this));
  if (typeof name === 'string' && context == null) callback = void 0;
  return on.call(this, events, callback, context);
};

// Inversion-of-control versions of `once`.
function listenToOnce(obj, name, callback) {
  // Map the event into a `{event: once}` object.
  var events = eventsApi(onceMap, {}, name, callback, _.bind(stopListening, this, obj));
  return listenTo.call(this, obj, events);
};
